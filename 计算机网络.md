...
通过调整CongWin(拥塞窗口) & Threshold(警戒值)实现
...
- tcp的三次握手**发生在客户端connect步骤中**
- 服务端accpet只是**从内核中已完成连接队列取出1个连接，与三次握手无关**
  - 已完成连接队列（FIFO with established state）



四次挥手**发生在主动调用close的一端**，对端关闭后，**己端read返回0**



#### 07 Tcp判断连接关闭 & read返回值详解🌼

网络编程中，**read返回0表明对端关闭连接**
...
- **对于阻塞I/O**（默认）
  ...
  - read返回值 = 0：**对端关闭连接**
  - read返回值 = -1时
    - **`erro == EINTR`**：**读取过程中发生中断**，应继续read
    - **其余情况应当断开连接**
- **对于非阻塞I/O**（设置方法：**`fcntl(fd, F_SETFL, fcntl(fd, F_GETFL, 0) | O_NONBLOCK)`**）
  - read返回值 > 0：正常读取数据
  - read返回值 = 0：**对端关闭连接**
  - read返回值 = -1时
    - errno == EAGAIN（**经常发生**）：非阻塞模式下**无可读数据**，应继续read
    - errno == EINTR ：中断，继续read
    - errno == EWOULDBLOCK（期望阻塞）
    - **其余情况应当断开连接**
...
攻击端**利用TCP三次握手机制**，发送大量**伪造IP的SYN报文，使服务器资源耗尽**

- **攻击原理**
  - 攻击端伪造IP地址，向服务器发送SYN报文
  - **服务器的ACK+SYN报文得不到回复**
  - 服务器在等待回复的过程中**消耗了资源**，**无法受理正常客户的连接**
    - 等待时间为SYN timeout
  - 攻击端循环发送SYN报文，直至服务器资源耗尽
- **防御措施**
  - **降低SYN timeout的等待时间**
  - 使用SYN缓存验证
  - 使用防火墙
...
- **应用层** -- DNS服务器、FTP服务器、HTTP服务器、CDN、负载均衡设备
- **IP层** -- 路由器、防火墙、NAT路由器
- **链路层** -- 交换机、网卡
- **物理层** -- 同轴电缆、双绞线、光纤、Hub
...
**Domain Name Systerm** 负责将域名解析为IP

- DNS位于应用层，本身事务性较强，**一般采用UDP报文承载**

- DNS使用TCP和UDP的53号端口
- 辅助域名服务器向权威域名服务器请求同步数据时，优先保证传输可靠性，采用TCP传输



#### 11 正向代理与反向代理

**正向代理** -- proxy代替client访问server，作用包括

- 隐藏client身份
- 替client访问自身不具备权限的资源，例如挂vpn上外网

**反向代理** -- proxy代替server，受理client请求，作用为

- 隐藏并保护server，防范Dos等攻击
- 分布式服务器的负载均衡，例如Nginx反向代理



#### 12 OSI七层模型与TCP/IP五层模型
...
<img src="https://figure-bed-zwd.oss-cn-hangzhou.aliyuncs.com/img_for_markdown/image-20220303101917854.png" alt="image-20220303101917854" style="zoom: 60%;" />



#### 13 浏览器输入URL到网页展示的全过程

- 在浏览器种输入URL
- DNS解析域名，获取对应的ip地址
- TCP三次握手建立连接
- 浏览器生成http request并发送
- 服务器处理请求，返回http response
- 浏览器渲染
- TCP四次挥手断开连接
...
- 通过**setsockopt可以修改缓冲区大小**

```bash
$ cat /proc/sys/net/core/rmem_default	# 查看tcp读缓冲区大小 默认128K
$ cat /proc/sys/net/core/wmem_default	# 查看tcp读缓冲区大小 默认16K
```

![image-20220906165917045](https://figure-bed-zwd.oss-cn-hangzhou.aliyuncs.com/img_for_markdown/image-20220906165917045.png)
...
- 发送方发送报文后，启动重传定时器，超出定时时间没有收到对应ack则触发超时重传
- 定时时间动态变化，默认为1s、3s、6s..

**坚持定时器**

- 接收方滑动窗口可用空间为0时，发送方停止数据发送并启动坚持定时器的轮询，直到发送方滑动窗口可用空间不为0
- 定时时间默认1.5s、3s、6s...

**keepalive定时器**

- 应用层没有数据进行传输时，每隔tcp_keepalive_time(默认2h)，发送一次保活心跳包
  - 如果发送成功则继续保持连接
  - 如果发送失败，则会多次尝试
  - 全部失败则认为连接关闭
...
- 主动断开连接的一方，第四次挥手后，会经历2MSL的TIME_WAIT状态
- 在此期间如果收到对方重传的FIN包，则重新挥手
