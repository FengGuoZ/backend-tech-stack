...
![image-20220929001727119](https://figure-bed-zwd.oss-cn-hangzhou.aliyuncs.com/img_for_markdown/image-20220929001727119.png)
...
![image-20220927105051339](https://figure-bed-zwd.oss-cn-hangzhou.aliyuncs.com/img_for_markdown/image-20220927105051339.png)
...
vector<int> missingTwo(vector<int>& nums) {
    ...
    int pivot = nSum / 2;                   // 拓扑关系为a<=pivot<b
    int leftSum = (1 + pivot) * pivot / 2;  // leftSum为1~pivot范围内所有数的加和
    for(int val : nums) {
        if(val <= pivot) {
            leftSum -= val;
        }
    }
    return {leftSum, nSum - leftSum};
}
```
...
![image-20220927162333278](https://figure-bed-zwd.oss-cn-hangzhou.aliyuncs.com/img_for_markdown/image-20220927162333278.png)

**算法思路**

维护大1个根堆+1个小根堆

- **大根堆用于存储 <= 中位数的一半数据**，堆顶为数组左侧**最大数**
- **小根堆用于存储 > 中位数的一半数据**，堆顶为数组右侧**最小数**
- 保证两者的**size差 < 1**
- **中位数备选**
  - 大根堆堆顶
  - 小根堆堆顶
